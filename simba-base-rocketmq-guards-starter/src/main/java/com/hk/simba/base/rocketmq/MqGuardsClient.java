...
public class MqGuardsClient {
    ...
    public void start() {
        ...
        // 发送超时时间，默认3秒
        properties.put("SendMsgTimeoutMillis",
            mqGuardsProperties.getSendMsgTimeoutMillis() == null ? 3000 : mqGuardsProperties.getSendMsgTimeoutMillis());
        producer = new ProducerImpl(properties).getDefaultMQProducer();
        // 同步发送重试次数，默认5次
        ...
        new FailureQueueHandleThread(failureQueue, this).start();
    ...
    public boolean saveMessageToServer(MqMessage mqMessage) {
        ...
        try {
            ...
            String result = HttpUtil.httpMethodPost(mqGuardsProperties.getServerUrl(), toJSONString(params), null);
    ...
    private SendResult sendMsg(MqMessage mqMessage, SendCallback sendCallback) {
        ...
        // 设置延迟时间
        Long delayTime = mqMessage.getDelayTime();
        if (nonNull(delayTime)) {
            Long startDeliverTime = System.currentTimeMillis() + delayTime;
            Date targetTime = new Date(startDeliverTime);
            mqMessage.setTargetTime(targetTime);
            message.putUserProperty(SystemPropKey.STARTDELIVERTIME, String.valueOf(startDeliverTime));
        }
        ...
        try {
            ...
            StoreFailedMessageToServerTask storeFailedMessageToServerTask =
                new StoreFailedMessageToServerTask(mqMessage, this);
