...
@Slf4j
@AllArgsConstructor
class FailureQueueHandleThread extends Thread {

    private final LinkedBlockingQueue<MqMessage> failMessageQueue;
    private final MqGuardsClient mqGuardsClient;
    ...
    public void run() {
        while (true) {
            try {
                ...
                int failQueueRetryTimes =
                    mqGuardsClient.getMqGuardsProperties().getFailQueueRetryTimes() == null ? 5 :
                        mqGuardsClient.getMqGuardsProperties().getFailQueueRetryTimes();
