...
mq-guards(开源版本)主要用于保障mq的发送成功，当mq发送失败时会进行失败记录的存储并补发
## 使用
...
<dependency>
    ...
    <artifactId>simba-base-rocketmq-guards-ons-client-starter</artifactId>
...
# 存储任务的后台接口地址
mq.guards.server-url=http://127.0.0.1:8080/mq/insert
...
# mq配置信息-mq发送超时时间(不设置默认为6000ms)
mq.guards.send-msg-timeout-millis=6000
# 由于后台通过线程池异步发送至存储任务进行存储操作，所以需要进行线程池的配置
# 核心线程池数
mq.guards.core-pool-size=2
# 最大线程池数
mq.guards.maximum-pool-size=10
# 线程存活时间
mq.guards.keep-alive-time=5*60*100
# 时间单位
mq.guards.time-unit=mills
# 队列数目
...
### 进行发送
``` java
   SendResult sendResult = mqGuardsClient.sendMessage(mqMessage);
```
### 需要关注的问题
